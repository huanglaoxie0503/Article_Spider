...
import logging
import pymysql
from Article_Spider.settings import mysql_host, mysql_user, mysql_password, mysql_db
...
class ArticleSpiderPipeline(object):
    ...
    def process_item(self, item, spider):
        try:
            # 插入
            if isinstance(item, ArticleSpiderQuestionItem):
                self.do_insert(item)
            elif isinstance(item, ArticleSpiderAnswerItem):
                self.do_insert(item)
            else:
